...
public class ChatScene implements P2PMessageListener {
    ...
    @FXML
    private ListView<String> groupOverviewListView;
    @FXML
    ...
    @FXML
    private Label typingStatusLabel;
    @FXML
    ...
    @FXML
    private Label currentUserStatusLabel;
    @FXML
    private Circle userAvatarCircle;
    @FXML
    private ToggleButton filterDMButton;
    @FXML
    private ToggleButton filterGroupButton;
    ...
    @FXML
    private Label groupSectionHeader;
    ...
    /**
     * Auto-connect to all friends to receive real-time online/offline status via
     * UDP heartbeat
     */
                ...
                try {
                    ...
                    if (friendP2PInfo != null) {
                        ...
                        if (friend != null) {
                            ...
                            if (connected) {
                                System.out.println("[ChatScene] ✅ Auto-connected to " + friend.getUsername()
                                        + " for status updates");
                            }
                        }
                    }
                ...
                public void onSyncCompleted(int successful, int failed) {
                    Platform.runLater(() -> {
                        if (failed == 0) {
                            ...
                            typingStatusLabel
                                    .setText("⚠️ Đồng bộ: " + successful + " thành công, " + failed + " thất bại");
                        }
                    });
                }
            });
    ...
    /**
     * Update online/offline status in conversation list (real-time via UDP
     * heartbeat)
     */
    private void updateConversationStatus(Long userId, boolean online) {
        ...
        for (int i = 0; i < conversationListView.getItems().size(); i++) {
            ...
            if (item.getUserId().equals(userId)) {
                ...
                System.out.println(
                        "[ChatScene] 🔄 Updated status for " + item.getName() + ": " + (online ? "ONLINE" : "OFFLINE"));
                break;
            }
        }
    }
    ...
    private void handleSendMessage() {
        ...
        if (convKey != null) {
            try {
                ...
                System.out.println(
                        "  - Encrypted: " + finalContent.substring(0, Math.min(30, finalContent.length())) + "...");
        ...
        if (hasP2P) {
            // Priority 1: Send via P2P with ENCRYPTED content + E2EE fields
            System.out.println("[ChatScene] 📤 Sending ENCRYPTED message via P2P (TRUE E2EE)");

            // ✅ CREATE P2PMessage WITH E2EE FIELDS
            ...
            // ✅ SEND P2PMessage (preserves E2EE fields)
            p2pManager.sendP2PMessage(friendId, p2pMessage).thenAccept(p2pSuccess -> {
                if (p2pSuccess) {
                    System.out.println("[ChatScene] ✅ Encrypted P2P message sent (TRUE E2EE)");

                    // ✅ FIX: Queue ONCE - only when P2P succeeds
    ...
    private void sendToServerAsync(MessageDTO messageDTO, boolean showStatus) {
        ...
        System.out.println(
                "  - MessageDTO: SenderId=" + messageDTO.getSenderId() + ", ReceiverId=" + messageDTO.getReceiverId());
    ...
    private void handleCreateGroup() {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/com/example/fxml/dialog/create-group-dialog.fxml"));
            ...
            // Load CSS
            dialogRoot.getStylesheets()
                    .add(getClass().getResource("/com/example/css/create-group-dialog.css").toExternalForm());
            ...
            if (dialogController.isConfirmed()) {
                ...
                System.out
                        .println("[ChatScene] Creating group: " + groupName + " with " + memberIds.size() + " members");
