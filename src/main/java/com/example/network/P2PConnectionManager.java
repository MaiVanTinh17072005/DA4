...
public class P2PConnectionManager {
        ...
        serverThread = new Thread(() -> {
            ...
            while (running) {
                try {
                    ...
                    System.out.println(
                            "[P2PConnectionManager] 📥 Incoming connection from: " + clientSocket.getInetAddress());
    ...
    public boolean connectToPeer(Long peerId, String ipAddress, int port) {
        ...
        try {
            System.out.println(
                    "[P2PConnectionManager] 🔗 Connecting to peer " + peerId + " at " + ipAddress + ":" + port);
    ...
    public CompletableFuture<Boolean> sendMessage(Long peerId, P2PMessage message) {
        ...
        try {
            ...
            System.out.println(
                    "[P2PConnectionManager] 📤 Sent message to peer " + peerId + ": " + message.getMessageId());
            ...
            if (message.isRequiresAck()) {
                ...
                ackFuture.orTimeout(ACK_TIMEOUT_MS, TimeUnit.MILLISECONDS)
                        .whenComplete((acked, error) -> {
                            ...
                            if (error != null) {
                                System.err.println(
                                        "[P2PConnectionManager] ⏱️ ACK timeout for message: " + message.getMessageId());
                                future.complete(false);
                            } else {
    ...
    private void handleIncomingConnection(Socket socket) {
        try {
            ...
            // Create session
            PeerSession session = new PeerSession(peerId, socket.getInetAddress().getHostAddress(), socket.getPort(),
                    0);
            session.setMyKeyPair(myKeyPair);
            session.setState(ConnectionState.CONNECTED);
            peerSessions.put(peerId, session);
    ...
    private void exchangeKeys(Long peerId, ObjectOutputStream out, ObjectInputStream in) throws Exception {
        ...
        System.out.println("[P2PConnectionManager] 📤 Sent my public key to peer: " + peerId);
        ...
        System.out.println("[P2PConnectionManager] 📥 Received peer's public key from: " + peerId);

        // Derive shared AES session key from both public keys
        SecretKey sessionKey = P2PEncryption.deriveSharedAESKey(myKeyPair.getPublic(), peerPublicKey);
        ...
        System.out.println("[P2PConnectionManager] 🔑 Derived shared AES session key");
        System.out.println("[P2PConnectionManager] My Public Key: " +
                java.util.Base64.getEncoder().encodeToString(myKeyPair.getPublic().getEncoded()).substring(0, 20)
                + "...");
        System.out.println("[P2PConnectionManager] Peer Public Key: " +
                java.util.Base64.getEncoder().encodeToString(peerPublicKey.getEncoded()).substring(0, 20) + "...");
    }

    private void exchangeKeysIncoming(Long peerId, P2PMessage firstMessage, ObjectOutputStream out,
    ...
    private void processMessage(Long peerId, P2PMessage message) {
        try {
            ...
            if (message.getSignature() != null) {
                String signatureData = message.getMessageId() + message.getTimestamp();
                boolean valid = P2PEncryption.verifySignature(signatureData, message.getSignature(),
                        session.getPeerPublicKey());
                if (!valid) {
                    System.err.println(
                            "[P2PConnectionManager] ⚠️ Invalid signature for message: " + message.getMessageId());
                    return;
                }
            }
            ...
            switch (message.getMsgType()) {
                ...
                case "ack":
                    System.out.println(
                            "[P2PConnectionManager] ✅ Received ACK for message: " + message.getAckForMessageId());
